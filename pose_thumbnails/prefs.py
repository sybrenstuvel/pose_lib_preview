...
class PoseThumbnailsPreferences(bpy.types.AddonPreferences):
    ...
    def draw(self, context):
        ...
        layout.separator()
        col = layout.box()
        col.label(text='Character Name and Pose Library recognition:', icon='TRIA_RIGHT')
        ...
        try:
            ...
            col.label(text='Error in regular expression: %s at position %s' % (ex.msg, ex.pos),
                      icon='ERROR')
        else:
            from . import core
            char = self.optional_name_prefix + 'Alpha_monster-blenrig.001'
            pl = core.pose_library_name_prefix(char, context)
            col.label(text='Object %r will use Pose Libraries %r' % (char, pl + '…'))
